...
type
  ...
  private
    ...
    procedure day8part1;
    procedure day8part2;
    function runTheProgram(inputArray: TStringArray):boolean;
...
if OpenDialog1.Execute then
  ...
  runTheProgram(instructions);
  end;
end;

procedure TForm1.day8part2;
var
  instructions, modified: TStringArray;
  i,pos:integer;
  programEnds: boolean;
  instruction:string;
  instructionSplit:TStringArray;
begin
if OpenDialog1.Execute then
  begin
  instructions:= openfileasArray(opendialog1.FileName, #$0A);
  setLength(modified, length(instructions));
  lbResults.Clear;
  //set modified to be the same as instructions
  for i:=0 to length(instructions)-1 do modified[i]:=instructions[i];

  //This time, we need an array to keep track of which instruction we have modified
  pos:=0;
  programEnds:=false;
  while not programEnds do
    begin
    instruction:=modified[pos];
    instructionsplit:=instruction.Split(' ');
    if (instructionsplit[0]='jmp') then
      begin
      modified[pos]:='nop '+instructionsplit[1];
      lbresults.items.add('changing jmp to nop at pos: '+inttostr(pos));
      end
    else if (instructionsplit[0]='nop') then
      begin
      modified[pos]:='jmp '+instructionsplit[1];
      lbresults.items.add('changing nop to jmp at pos: '+inttostr(pos));
      end;
    programEnds:=runTheProgram(modified);
    //reset the element we changed
    modified[pos]:=instructions[pos];
    if programEnds = false then pos:=pos +1;
    end;
  end;
end;

function TForm1.runTheProgram(inputArray: TStringArray):boolean;
var
  visited: TIntArray;
...
begin
  setlength(visited, length(inputArray));
  ...
  moveBy:=1;
    while not done do
      ...
      pos:=pos + moveBy;
      if (pos = length(visited)) then result:=true else result:=false;
    end;
end;
